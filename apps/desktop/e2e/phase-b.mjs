// Phase B: the fixes from phase A's screenshots, a changed host key, and
// deleting a host.
...
await page.waitFor(
  `document.querySelector('.modal-title')?.textContent === 'Der Host-Key hat sich geändert'`,
...
check(
  'replacing is impossible until the address is typed',
  await page.eval(
    `[...document.querySelectorAll('.modal-footer button')].find(b => b.textContent.includes('übernehmen'))?.disabled === true`,
  ),
);
check(
  'focus sits on the safe choice',
  (await page.eval(`document.activeElement?.textContent`)) === 'Nicht verbinden',
...
await page.type('127.0.0.2');
check(
  'a wrong address keeps the button disabled',
  await page.eval(
    `[...document.querySelectorAll('.modal-footer button')].find(b => b.textContent.includes('übernehmen'))?.disabled === true`,
  ),
);
await page.fill('.modal input', '127.0.0.1');
await page.click('.modal-footer button', 'Neuen Schlüssel übernehmen');

await page.waitFor(`document.querySelector('.modal-title')?.textContent === 'Passwort'`, {
  what: 'password prompt after replacing',
  timeout: 15_000,
});
check('after replacing the key, login continues with the password', true);
await page.type('nyu');
await page.key('Enter');
...
const dot = `[...document.querySelectorAll('.host-row')].find(r => r.textContent.includes('dev-sshd'))?.querySelector('.dot')?.dataset.state`;
...
// ── Delete the host ─────────────────────────────────────────────────────────
await page.eval(
  `[...document.querySelectorAll('.host-row')].find(r => r.textContent.includes('dev-sshd')).querySelector('.host-edit').click()`,
);
await page.waitFor(
  `document.querySelector('.modal-title')?.textContent === 'dev-sshd bearbeiten'`,
  { what: 'edit form' },
);
